...
public class Program {
    ...
    /**
     ...
     5. Написать функцию, создающую резервную копию всех файлов в директории(без поддиректорий) во вновь созданную папку ./backup
     6. Доработайте класс Tree и метод print который мы разработали на семинаре. Ваш метод должен распечатать полноценное дерево директорий и файлов относительно корневой директории.
     7***. Предположить, что числа в исходном массиве из 9 элементов имеют диапазон[0, 3], и представляют собой, например, состояния ячеек поля для игры в крестикинолики, где 0 – это пустое поле, 1 – это поле с крестиком, 2 – это поле с ноликом,
...
//        getBackUp();
    ...
    /**
     * Метод поиска слова в файлах директории с использованием массива имен файлов
     * @param files файлы директории
     * @param search слово которое ищем
     * @return возвращаем массив имен файлов содержащих искомое слово
     * @throws IOException
     */
    ...
    /**
     * TODO желательно сделать копирование из поддерикторий (либо с ними, либо в один католог), dir может кинуть nullpointer если останется время
     *
     *
     * Функция копирования файлов без поддерикторий в папку "/backup"
     *
     * @throws IOException
     */
    private static void getBackUp() throws IOException {
        File path = new File(new File(".").getCanonicalPath());
        File newDir = new File(path+"/backup");
        newDir.mkdir();
        File[] dir = path.listFiles();
        for (int i = 0; i < dir.length; i++) {
            if (dir[i].isFile()){
                try(FileInputStream is = new FileInputStream(dir[i]);
                    FileOutputStream os = new FileOutputStream(new File(newDir, dir[i].getName()));){
